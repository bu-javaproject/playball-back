...
public interface ComplimentRepository extends JpaRepository<Compliment, Long> {
    ...
    @Query("SELECT c FROM Compliment c WHERE c.rateeId = :memberId AND (:cursor IS NULL OR c.complimentId < :cursor) ORDER BY c.complimentId DESC")
    List<Compliment> findReceivedByMember(@Param("memberId") Long memberId,
